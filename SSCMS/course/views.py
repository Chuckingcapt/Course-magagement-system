...
from constants import INVALID_KIND, INVALID_REQUEST_METHOD, COURSE_FULL
        ...
        if course.status == 4:
            ...
            for sc in scs:
                if sc.scores is None:
                    all_given = False
                    res += "<div>%s has not been graded</div>" % sc.student
...
def view_course(request, view_kind):
    """
    :param view_kind:
        current: Check current courses
        is_end: Check ended courses
        select: Select courses
        withdraw: Drop courses
    """
